...
class LevelProbe:
    ...
    def orbit_block_scan(self):
        """Sense 4: Block decomposition of ALL orbits' self-coupling.

        For each orbit with k>1 blocks:
          - Self-overlap spectrum (rank, null)
          - Diagonal block rank/null
          - Off-diagonal structure (flat/structured)
          - Block-circulant Fourier reconstruction
          - Null per channel (democratic check)
        """
        k = self.orbit_size
        if k is None or k < 2:
            return

        print(f"\n  ORBIT BLOCK SCAN (Sense 4):")

        # Compute generator once
        ord2 = 1
        val = 2
        while val != 1:
            val = (val * 2) % self.p
        ...
        exp = ord2 // k
        g = pow(2, exp, self.p)
        print(f"    Generator: g = 2^{exp} = {g} mod {self.p}")

        def apply_perm(m, gg):
            new_edges = set()
            for a, b in m:
                na = (gg * a) % self.p if a < self.p else a
                nb = (gg * b) % self.p if b < self.p else b
                new_edges.add((min(na, nb), max(na, nb)))
            return frozenset(new_edges)
        ...
        for oi, (oms, oidx) in enumerate(self.orbits):
            k_orb = len(oms)
            is_fp = (oi == self.fp_orbit_idx)
            is_vac = (oi == self.vac_orbit_idx)

            if is_fp or k_orb < 2:
                # FP: single block, no circulant structure
                orb_ms = [self.match_sets[i] for i in oidx]
                O = build_overlap(orb_ms, orb_ms, self.nv, same=True)
                evals = np.linalg.eigvalsh(O)[::-1]
                bs = len(oidx)
                rank = int(np.sum(np.abs(evals) > 1e-6))
                null = bs - rank
                scan_results.append({
                    'orbit': oi, 'is_vac': is_vac, 'is_fp': True,
                    'k_orb': 1, 'bs': bs, 'bs_blk': bs,
                    'rank': rank, 'null': null,
                    'diag_rank': rank, 'diag_null': null,
                    'half_rank_orbit': (null == bs // 2),
                    'half_rank_diag': (null == bs // 2),
                    'fourier_ok': None,
                    'democratic': None,
                    'null_per_ch': None,
                    'diag_frac': 1.0,
            ...
            # Multi-block orbit
            block_local_idx = []
            for bi in range(k_orb):
                block_local_idx.append(self.blocks[oms[bi]])
            bs_blk = len(block_local_idx[0])
            bs = len(oidx)

            # Full orbit self-overlap
            orb_ms = [self.match_sets[i] for i in oidx]
            O_full = build_overlap(orb_ms, orb_ms, self.nv, same=True)
            evals_full = np.linalg.eigvalsh(O_full)[::-1]
            rank_full = int(np.sum(np.abs(evals_full) > 1e-6))
            null_full = bs - rank_full

            # Diagonal block (representative block 0)
            ms_0 = [self.match_sets[i] for i in block_local_idx[0]]
            O_diag = build_overlap(ms_0, ms_0, self.nv, same=True)
            evals_diag = np.linalg.eigvalsh(O_diag)[::-1]
            diag_rank = int(np.sum(np.abs(evals_diag) > 1e-6))
            diag_null = bs_blk - diag_rank

            # Frobenius: diagonal
            diag_frob2 = 0
            for bi in range(k_orb):
                ms_bi = [self.match_sets[i] for i in block_local_idx[bi]]
                Od = build_overlap(ms_bi, ms_bi, self.nv, same=True)
                diag_frob2 += np.sum(Od ** 2)

            # Off-diagonal: shifts 1..k-1
            offdiag_frob2 = 0
            n_flat_offdiag = 0
            for shift in range(1, k_orb):
                ms_j = [self.match_sets[i] for i in block_local_idx[shift]]
                O_off = build_overlap(ms_0, ms_j, self.nv)
                svs = np.linalg.svd(O_off, compute_uv=False)
                svs_nz = svs[svs > 1e-10]
                f2 = np.sum(svs ** 2)
                offdiag_frob2 += f2 * k_orb  # k copies per shift
                if len(svs_nz) > 0 and svs_nz[0] / svs_nz[-1] < 1.001:
                    n_flat_offdiag += 1

            full_frob2 = np.sum(O_full ** 2)
            diag_frac = diag_frob2 / full_frob2 if full_frob2 > 0 else 0
            ...
            block_seq = [0]
            cur_blk = 0
            for _ in range(k_orb - 1):
                m_cur = block_match_lists[cur_blk][0]
                m_next = apply_perm(m_cur, g)
                found = False
                for bi in range(k_orb):
                    if m_next in block_match_lists[bi]:
                        cur_blk = bi
            ...
            if block_seq is not None and len(set(block_seq)) == k_orb:
                # Build aligned orderings
                aligned_orders = {0: list(range(bs_blk))}
                aligned_ok = True
                for step in range(1, k_orb):
                    bi = block_seq[step]
                    order_bi = []
                    for mi, m in enumerate(block_match_lists[0]):
                        cur = m
                        for _ in range(step):
                            cur = apply_perm(cur, g)
                        try:
                            idx = block_match_lists[bi].index(cur)
                            order_bi.append(idx)
                        except ValueError:
                ...
                if aligned_ok:
                    ...
                    blocks_B = []
                    ms_0_sets = [self.match_sets[block_local_idx[0][i]]
                                 for i in range(bs_blk)]
                    for step in range(k_orb):
                        bi = block_seq[step]
                        B = np.zeros((bs_blk, bs_blk))
                        ms_j = [self.match_sets[
                                    block_local_idx[bi][aligned_orders[bi][j]]]
                                for j in range(bs_blk)]
                        for ii in range(bs_blk):
                            for jj in range(bs_blk):
                                if step == 0 and ii == jj:
                                    B[ii, jj] = self.nv
                                else:
                                    B[ii, jj] = 2 * len(ms_0_sets[ii] & ms_j[jj])
                        blocks_B.append(B)

                    omega = np.exp(2j * np.pi / k_orb)
                    all_evals_recon = []
                    null_per_ch = []

                    for ell in range(k_orb):
                        M = np.zeros((bs_blk, bs_blk), dtype=complex)
                        for j in range(k_orb):
                            M += blocks_B[j] * (omega ** (j * ell))
                        M_herm = (M + M.conj().T) / 2
                        evals_ch = np.linalg.eigvalsh(M_herm)[::-1]
                        all_evals_recon.extend(evals_ch.tolist())
                        null_ch = bs_blk - int(np.sum(np.abs(evals_ch) > 1e-6))
                        null_per_ch.append(null_ch)

                    recon_sorted = np.sort(all_evals_recon)[::-1]
                    full_sorted = np.sort(evals_full)[::-1]
                    max_diff = np.max(np.abs(recon_sorted - full_sorted))
                    fourier_ok = (max_diff < 1e-8)

                    if null_per_ch and len(set(null_per_ch)) == 1:
                        democratic = True

            scan_results.append({
                'orbit': oi, 'is_vac': is_vac, 'is_fp': False,
                'k_orb': k_orb, 'bs': bs, 'bs_blk': bs_blk,
                'rank': rank_full, 'null': null_full,
                'diag_rank': diag_rank, 'diag_null': diag_null,
                'half_rank_orbit': (null_full == bs // 2),
                'half_rank_diag': (diag_null == bs_blk // 2),
                'fourier_ok': fourier_ok,
                'democratic': democratic,
                'null_per_ch': null_per_ch,
                'diag_frac': diag_frac,
                'n_flat_offdiag': n_flat_offdiag
            })

        # ---- Summary table ----
        print(f"\n    {'Orb':>4} {'Tag':>4} {'bs':>5} {'blk':>4} "
              f"{'rank':>5} {'null':>5} {'½?':>3} "
              f"{'d_rk':>5} {'d_nl':>5} {'d½?':>3} "
              f"{'Four':>4} {'Demo':>4} "
              f"{'D%':>6} {'flat':>4} "
        ...
        for r in scan_results:
            tag = 'VAC' if r['is_vac'] else ('FP' if r['is_fp'] else '')
            hr = 'Y' if r['half_rank_orbit'] else 'N'
            dhr = 'Y' if r['half_rank_diag'] else 'N'
            ...
            if r['is_fp']:
                ...
                four_str = 'Y' if r['fourier_ok'] else 'N'
                demo_str = 'Y' if r['democratic'] else 'N'
                if r['null_per_ch'] is not None:
                    uniq = sorted(set(r['null_per_ch']))
                    null_ch_str = ','.join(str(x) for x in uniq)
                else:
                    null_ch_str = 'fail'

            print(f"    {r['orbit']:>4} {tag:>4} {r['bs']:>5} {r['bs_blk']:>4} "
                  f"{r['rank']:>5} {r['null']:>5} {hr:>3} "
                  f"{r['diag_rank']:>5} {r['diag_null']:>5} {dhr:>3} "
                  f"{four_str:>4} {demo_str:>4} "
                  f"{r['diag_frac']:>5.1%} {r['n_flat_offdiag']:>4} "
                  f"{null_ch_str:>8}")

        # ---- Aggregate statistics ----
        multi = [r for r in scan_results if not r['is_fp']]
        n_half_rank = sum(1 for r in multi if r['half_rank_orbit'])
        n_diag_half = sum(1 for r in multi if r['half_rank_diag'])
        n_fourier = sum(1 for r in multi if r['fourier_ok'])
        n_demo = sum(1 for r in multi if r['democratic'])

        print(f"\n    AGGREGATE ({len(multi)} multi-block orbits):")
        print(f"      Orbit half-rank: {n_half_rank}/{len(multi)}")
        print(f"      Diag block half-rank: {n_diag_half}/{len(multi)}")
        print(f"      Fourier exact: {n_fourier}/{len(multi)}")
        print(f"      Democratic null: {n_demo}/{len(multi)}")

        # ---- Detailed output for non-vacuum orbits with null space ----
        interesting = [r for r in multi
                       if not r['is_vac'] and r['null'] > 0]
        if interesting:
            print(f"\n    DETAIL: Non-vacuum orbits with null space:")
            for r in interesting:
                ch_str = str(r['null_per_ch']) if r['null_per_ch'] else 'N/A'
                print(f"      Orbit {r['orbit']}: null={r['null']}, "
                      f"diag_null={r['diag_null']}, "
                      f"null/ch={ch_str}, "
                      f"demo={'YES' if r['democratic'] else 'NO'}")

        self.results['orbit_scan'] = scan_results
