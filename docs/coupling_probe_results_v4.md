...
Standard RG starts with a UV theory and integrates out modes, losing resolution at each step. This framework operates in reverse: each K_{2n} adds irreducible arithmetic content invisible at lower levels. K_6 is featureless (full rank, no null space). K_8 introduces the half-rank split and independent CP. K_10 breaks K_8's accidental degeneracies. K_12 resolves further with 26 orbits and 8 distinct eigenvalue levels.

Galois disjointness guarantees each level's contribution is genuinely independent -- the number field at K_12 cannot be derived from K_8's. The framework doesn't integrate out. It integrates in.

The CP independence result sharpens this: going up the chain, most levels *lose* the independent CP quantum number (it collapses into the rotation). K_8 is the unique prime-p level where matter/antimatter is a genuinely separate degree of freedom. The fermion level has *more* structure than the levels above it -- the opposite of standard coarse-graining.
